...
class IOHMM_model:
    def __init__(self, num_states, inputs, outputs, max_iter, tol, initial_pi=None, transition_matrix=None, emission_matrix=None, lsd=None):
        ...
        if lsd is not None:
            ...
            self.lsd = nn.Parameter(torch.log(5.0*torch.ones(num_states)), requires_grad=True)


    def softmax_(self, input, state):
        with torch.no_grad():
            # I think is dim = 0
            # return the log_probs
            return (self.transition_matrix[state,:,:] @ torch.cat((torch.tensor([1.0]),input))) - torch.logsumexp(self.transition_matrix[state,:,:] @ torch.cat((torch.tensor([1.0]),input)), dim =0)
    ...
    def softmax(self, input):
        with torch.no_grad():
            ...
            softmax_probs = transition_logits - torch.logsumexp(transition_logits, dim=1)
            return softmax_probs

    def dnorm(self, x, mean, lsd):
        # !!!!!!!!!!!! RETURNS THE LOG OF THE DENSITY NOT THE DENSITY ITSELF !!!!!!!!!!!!!!!
        with torch.no_grad():
        
            return (-0.5 * ((x - mean) / torch.exp(lsd)) ** 2) - (lsd + torch.log(torch.sqrt(torch.tensor(2 * np.pi))))
    ...
    def _forward(self):
        with torch.no_grad():
            ...
            for t in range(1, T):
                ...
                #------ not so sure about this to be a logsumexp -------------
                transition_prob = torch.logsumexp(transition_prob, axis=0)
    ...
    def _log_likelihood(self, gamma, xi):
        ...
        for t, output in enumerate(self.outputs):
            ...
            log_dnorm = self.dnorm(x, mu, self.lsd)
    ...
    def _baum_welch(self):
        ...
        optimizer = optim.SGD([self.initial_pi, self.transition_matrix, self.emission_matrix, self.lsd], lr=5.0)
        scheduler = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.99)  # Decrease lr by 0.9 every 1 steps
    ...
    def viterbi(self):
        with torch.no_grad():
            ...
            transition_prob = self.softmax(U[-2]) + alpha[-2].unsqueeze(1)
            transition_prob = torch.logsumexp(transition_prob, axis=0)
            last_state = torch.argmax(alpha[-1] + transition_prob)
            path.append(last_state.item())

            for i in reversed(range(len(self.outputs) - 1)):
                transition_prob = self.softmax(U[i]) + alpha[i].unsqueeze(1)
                transition_prob = torch.logsumexp(transition_prob, axis=0)
                last_state = torch.argmax(transition_prob + alpha[i+1])
                path.append(last_state.item())
    ...
    def predict(self, u_t1):
        ...
        print(rho_t)
        # Compute rho_i_t+1
        rho_t1 = torch.logsumexp(self.softmax(u_t1) + rho_t, axis=1)
        print(rho_t1)
        # Compute eta_i_t
        eta_i_t1 = self.emission_matrix @ torch.cat((torch.tensor([1.0]), u_t1))
        print(eta_i_t1)
        # Compute eta_t+1
        eta_t1 = torch.sum(torch.exp(rho_t1) * eta_i_t1)
